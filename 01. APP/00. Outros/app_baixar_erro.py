...
def normalizar_hfacs_caixas(hfacs_caixas):
    ...
    for chave in HFACS_CAIXAS_ORDEM:
        valores = hfacs_caixas.get(chave, [])
...
def montar_substituicoes_word(dados_analista, dados_basicos, descricao_detalhada, texto_final):
    titulo = primeiro_valor(
        dados_basicos.get("Arquivo-base"),
        dados_basicos.get("Local do acidente"),
        padrao="RELATÓRIO DE INVESTIGAÇÃO DE ACIDENTE",
    )
...
def preencher_caixas_hfacs(document_xml, hfacs_caixas):
    ...
    # O modelo possui uma versão DrawingML e uma versão VML das mesmas 69 caixas.
    # As duas são preenchidas para manter compatibilidade no Word e na conversão para PDF.
    secoes_visuais = [
        [
            "governo_orgaos_reguladores",
            "influencias_politicas_economicas",
    ...
    if len(conteudos_caixas) >= 140:
        # Após as duas caixas do cabeçalho, o Word mantém cada caixa do diagrama em par:
        # uma versão DrawingML e uma versão VML. As duas recebem o mesmo texto.
        caixas_diagrama = conteudos_caixas[2:140]
        for indice, valor in enumerate(valores):
            for caixa in caixas_diagrama[indice * 2 : indice * 2 + 2]:
...
def converter_docx_bytes_para_pdf_bytes(docx_bytes):
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        docx_path = tmp / "relatorio_hfacs.docx"
        docx_path.write_bytes(docx_bytes)

        soffice = shutil.which("libreoffice") or shutil.which("soffice")
        if not soffice:
            raise RuntimeError("LibreOffice não encontrado para converter DOCX em PDF.")
        ...
        pdf_path = tmp / "relatorio_hfacs.pdf"
        if not pdf_path.exists():
            raise RuntimeError("A conversão para PDF não gerou o arquivo esperado.")
...
if "relatorio_editavel" in st.session_state:
    ...
    if not modelo_word:
        ...
        try:
            ...
            pdf_bytes = converter_docx_bytes_para_pdf_bytes(docx_bytes)
