...
SUPORTE_DIR = ROOT_DIR / "00. Suporte ao aplicativo"
...
def completar_caixas_hfacs_por_texto(hfacs_caixas, texto_final):
    resultado = normalizar_hfacs_caixas(hfacs_caixas)
    chaves_atos = ["erros_decisao", "erros_baseados_habilidade", "erros_percepcao", "rotina", "excepcional"]
    if any(any(resultado[chave]) for chave in chaves_atos):
        return resultado

    itens_atos = extrair_itens_secao(
        texto_final,
        termos_inicio=["Atos inseguros", "Nível 1", "Nivel 1", "Ato inseguro"],
        termos_fim=["Recomendações", "Recomendacoes", "Considerações finais", "Consideracoes finais", "Resumo"],
        max_itens=9,
    )

    # Se o relatório textual possui atos inseguros, aproveita o texto já gerado para preencher
    # pelo menos as caixas do nível 1 no Word/PDF. Não cria fatos novos.
    if itens_atos:
        distribuicao = [
            "erros_decisao",
            "erros_baseados_habilidade",
        ...
        for i, item in enumerate(itens_atos[:15]):
            chave = distribuicao[i % len(distribuicao)]
            pos = i // len(distribuicao)
            if pos < 3 and not resultado[chave][pos]:
                resultado[chave][pos] = item
