...
MODELO_WORD_NOME_ATUAL = "MODELO_WORD2_caixas_texto(1)(7).docx"
...
def montar_substituicoes_word(dados_analista, dados_basicos, descricao_detalhada, texto_final):
    ...
    return {
        ...
        # Marcador da página 2 no modelo mais recente
        "ESCREVA EM NO MAXIMO QUATRO PALAVRAS -COLOQUE AQUI UMA NOME ADEQUADO PARA A INVESTIGAÇÃO QUE FAÇA REFERENCIA AO TIPO DE ACIDENTE E AO LOCAL DO ACIDENTE - ": titulo,
        "NÃO COPIAR O NOME DO ARQUIVO ANEXADO": "",
...
def montar_valores_hfacs_detalhado(hfacs_caixas):
    """Monta os 69 valores da página de classificação detalhada (3 caixas por subtópico)."""
    secoes_visuais = [
        [
            "governo_orgaos_reguladores",
            "influencias_politicas_economicas",
...
def montar_resumo_hfacs_para_pagina_4(hfacs_caixas):
    """Monta valores resumidos para os bloquinhos da página 4.

    A página 4 possui menos bloquinhos que a página detalhada. Quando houver mais
    fatores do que espaços, os fatores são resumidos e combinados no mesmo bloquinho,
    sem usar reticências e sem criar conteúdo novo.
    """
    grupos = {
        "FATORES EXTERNOS": [
            "governo_orgaos_reguladores",
    ...
    def cortar_por_palavras(texto, max_palavras):
        palavras = re.findall(r"\S+", str(texto or "").strip())
        if len(palavras) <= max_palavras:
            return " ".join(palavras)
        return " ".join(palavras[:max_palavras])

    def resumir_fator_pagina_4(texto, max_palavras=7):
        """Resume um fator para caber na página 4 sem cortar com reticências."""
        texto = re.sub(r"\s+", " ", str(texto or "")).strip()
        texto = texto.replace("...", "")
        ...
        regras = [
            (r"(?i)^aus[eê]ncia de normas espec[ií]ficas para identifica[cç][aã]o de (.+)$", r"Ausência de padrão na identificação de \1"),
            (r"(?i)^aus[eê]ncia de normas espec[ií]ficas para identificar (.+)$", r"Ausência de padrão na identificação de \1"),
            (r"(?i)^aus[eê]ncia de normas espec[ií]ficas para (.+)$", r"Ausência de norma para \1"),
            (r"(?i)^falta de atualiza[cç][aã]o normativa para .+$", "Falta de atualização normativa"),
            (r"(?i)^falta de atualiza[cç][aã]o de normas para .+$", "Falta de atualização normativa"),
            (r"(?i)^aus[eê]ncia de procedimentos espec[ií]ficos para (.+)$", r"Ausência de procedimento para \1"),
            (r"(?i)^falta de procedimento espec[ií]fico para (.+)$", r"Falta de procedimento para \1"),
            (r"(?i)^inexist[eê]ncia de procedimento espec[ií]fico para (.+)$", r"Inexistência de procedimento para \1"),
            (r"(?i)^defici[eê]ncia de treinamento para (.+)$", r"Treinamento insuficiente para \1"),
            (r"(?i)^falha de comunica[cç][aã]o entre (.+)$", r"Falha de comunicação entre \1"),
            (r"(?i)^press[aã]o de produ[cç][aã]o para .+$", "Pressão de produção"),
            (r"(?i)^press[aã]o de prazo para .+$", "Pressão de prazo"),
        ]
        for padrao, substituto in regras:
            novo = re.sub(padrao, substituto, texto).strip()
            if novo != texto:
                texto = novo
                break

        # Remove explicações longas; mantém apenas o núcleo do fator.
        texto = re.split(
            r"\b(devido a|decorrente de|associad[oa] a|relacionad[oa] a|resultando em|comprometendo|o que|que levou|que favoreceu|por falta de)\b",
            texto,
            maxsplit=1,
            flags=re.IGNORECASE,
        )[0].strip(" ;:,.–—-")

        # Evita excesso de qualificadores que estoura o bloquinho.
        texto = re.sub(r"\b(específicas?|adequadas?|operacionais?|sistêmicas?|diretamente|claramente)\b", "", texto, flags=re.IGNORECASE)
        texto = re.sub(r"\s+", " ", texto).strip(" ;:,.–—-")
        return cortar_por_palavras(texto, max_palavras)

    def itens_do_grupo(chaves):
        itens = []
        vistos = set()
        for chave in chaves:
            for valor in hfacs_caixas.get(chave, ["", "", ""]):
                valor_limpo = resumir_fator_pagina_4(valor, max_palavras=8)
                if not valor_limpo:
                    continue
                assinatura = valor_limpo.lower()
                if assinatura not in vistos:
                    vistos.add(assinatura)
                    itens.append(valor_limpo)
        return itens

    def compactar_itens(itens, quantidade_caixas):
        if quantidade_caixas <= 0:
            return []
        if len(itens) <= quantidade_caixas:
            return [resumir_fator_pagina_4(item, max_palavras=8) for item in itens] + [""] * (quantidade_caixas - len(itens))

        tamanho_grupo = max(1, (len(itens) + quantidade_caixas - 1) // quantidade_caixas)
        caixas = []
        for i in range(0, len(itens), tamanho_grupo):
            grupo = itens[i:i + tamanho_grupo]
            partes = [resumir_fator_pagina_4(item, max_palavras=5) for item in grupo]
            partes = [p for p in partes if p]
            caixas.append("; ".join(partes))
            if len(caixas) == quantidade_caixas:
                break
        return caixas + [""] * (quantidade_caixas - len(caixas))

    return {nome: itens_do_grupo(chaves) for nome, chaves in grupos.items()}, compactar_itens
...
def preencher_caixas_hfacs(document_xml, hfacs_caixas):
    ...
    hfacs_caixas = normalizar_hfacs_caixas(hfacs_caixas)
    valores_detalhados = montar_valores_hfacs_detalhado(hfacs_caixas)
    ...
    def texto_caixa(caixa):
        return "".join(caixa.xpath(".//w:t/text()", namespaces=ns))
    ...
    def ordenar_por_posicao(pares):
        linhas = []
        tolerancia_y = 100000
        for item in sorted(pares, key=lambda item: item[0]):
        ...
        return caixas_ordenadas

    # Página 3: mantém a lógica já existente de preenchimento detalhado do HFACS.
    # No modelo novo, os 69 bloquinhos da página 3 aparecem como os 69 primeiros pares
    # DrawingML/VML dentro do XML do Word.
    pares_pagina_3 = []
    limite_pagina_3 = min(len(conteudos_caixas), 138)
    for indice in range(0, limite_pagina_3, 2):
        caixa = conteudos_caixas[indice]
        par_vml = conteudos_caixas[indice + 1] if indice + 1 < limite_pagina_3 else None
        coords = coordenadas_caixa(caixa)
        if coords is not None:
            pares_pagina_3.append((coords[0], coords[1], caixa, par_vml))

    if len(pares_pagina_3) >= 69:
        caixas_ordenadas = ordenar_por_posicao(pares_pagina_3)[:69]
        for (_, _, caixa_drawing, caixa_vml), valor in zip(caixas_ordenadas, valores_detalhados):
        ...
        for caixa, valor in zip(conteudos_caixas[:69], valores_detalhados):
            limpar_textos_caixa(caixa, valor)

    # Página 4: gráfico-resumo. Preenche os bloquinhos por categoria geral.
    # A página 4 tem menos bloquinhos; por isso os fatores excedentes são compactados.
    itens_por_grupo, compactar_itens = montar_resumo_hfacs_para_pagina_4(hfacs_caixas)

    marcadores_resumo = {
        "FATORES EXTERNOS": "FATORES EXTERNOS TEXTO AQUI AQUI",
        "INFLUÊNCIAS ORGANIZACIONAIS": "INFLUÊNCIAS ORGANIZACIONAIS TEXTO AQUI AQUI",
        "SUPERVISÃO INADEQUADA": "SUPERVISÃO INADEQUADA TEXTO AQUI AQUI",
        "CONDIÇÕES PRECURSORAS": "CONDIÇÕES PRECURSORAS TEXTO AQUI AQUI",
        "AÇÕES INSEGURAS": "AÇÕES INSEGURAS TEXTO AQUI AQUI",
    }

    for nome_grupo, marcador in marcadores_resumo.items():
        pares_grupo = []
        for indice, caixa in enumerate(conteudos_caixas):
            if marcador not in texto_caixa(caixa):
                continue
            # Ignora a cópia VML quando ela vem logo depois da DrawingML; ela será preenchida em par.
            if indice % 2 == 1 and indice > 0 and marcador in texto_caixa(conteudos_caixas[indice - 1]):
                continue
            coords = coordenadas_caixa(caixa)
            if coords is None:
                continue
            par_vml = conteudos_caixas[indice + 1] if indice + 1 < len(conteudos_caixas) else None
            pares_grupo.append((coords[0], coords[1], caixa, par_vml))

        pares_grupo = ordenar_por_posicao(pares_grupo)
        valores_resumo = compactar_itens(itens_por_grupo.get(nome_grupo, []), len(pares_grupo))
        for (_, _, caixa_drawing, caixa_vml), valor in zip(pares_grupo, valores_resumo):
            limpar_textos_caixa(caixa_drawing, valor)
            if caixa_vml is not None:
                limpar_textos_caixa(caixa_vml, valor)
    ...
    with zipfile.ZipFile(modelo_path, "r") as zin, zipfile.ZipFile(buffer_saida, "w", zipfile.ZIP_DEFLATED) as zout:
        for item in zin.infolist():
            ...
            if item.filename == "word/document.xml":
                # Primeiro preenche as caixas das páginas 3 e 4.
                # Depois substitui os demais marcadores do Word. Essa ordem evita que
                # os marcadores da página 4 sejam apagados antes do preenchimento.
